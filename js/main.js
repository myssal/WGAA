...
async function loadConfigs(region) {
  ...
  try {
    ...
    // Fetch Memory data
    const [equipDataRes, equipResRes, equipSuitRes, awarenessSettingRes] = await Promise.all([
    ...
    if (!equipDataRes.ok || !equipResRes.ok || !equipSuitRes.ok || !awarenessSettingRes.ok) throw new Error("Failed to fetch Memory config files");
    equips = await equipDataRes.json();
    equipRes = await equipResRes.json();
